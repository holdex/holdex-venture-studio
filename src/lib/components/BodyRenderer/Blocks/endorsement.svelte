<script lang="ts">
  import type { EndorsementElement } from '$components/BodyParser/blocks';
  import Link from '$components/BodyRenderer/Blocks/link.svelte';

  export let item: EndorsementElement;
</script>

<div
  class="endorsement-wrapper flex flex-col gap-4 w-full my-12 items-stretch smDown:items-start xl-up:py-10 xl-up:px-28 tb:p-20 pt-8 smDown:px-4"
>
  <div class="endorsement relative">
    <span
    ...
    <div class="author-info flex items-center justify-end gap-4 mt-4">
      ...
      <img src={item.picture?.url} alt={item.name} class="h-14 w-14 rounded-xl" />
    </div>
  </div>
</div>
